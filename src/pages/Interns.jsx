...
.tm-filter-bar { display: flex; align-items: center; gap: 14px; margin-bottom: 22px; }
...
.tm-filter-tabs { display: flex; gap: 4px; background: #F5F0E5; border-radius: 10px; padding: 3px; }
...
if (typeof document !== 'undefined' && !document.getElementById('int-tm-styles')) {
    const tag = document.createElement('style');
    tag.id = 'int-tm-styles';
...
const Interns = ({ onNavigate }) => {
  ...
  const response = await fetch(`${backendUrl}/interns`, {
    method: 'GET', 
    headers: {
  ...
  if (response.ok) {
    ...
    console.error("Server error or Unauthorized access");
  }
} catch (error) { 
  ...
  setLoading(false); 
    ...
    const handleLeaveToggle = async (member) => {
        if (member.status === 'On Leave') {
            try {
                ...
                setTeam(prev => prev.map(m => m.id === member.id ? { ...m, status: 'Active', leaveDays: undefined } : m));
    ...
    const handleEditPicUpload = (e) => {
        ...
        if (!file) return;
        if (file.size > 2 * 1024 * 1024) { showToast('Image 2MB se badi hai', 'error'); return; }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            setDeleteModal(null);
        } catch (error) { showToast('Failed to remove intern', 'error'); }
    };

    const resetForm = () => {
        setForm({ name: '', email: '', phone: '', type: 'Intern', role: '', rank: '', department: '', location: '', checkInTime: '09:00', checkOutTime: '17:30', education: '', institute: '', profilePic: null });
        setErrors({});
    };
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                        ...
                        <button type="button" className="tm-btn-cancel" style={{ fontSize: 12, padding: '6px 14px' }}
                            onClick={() => document.getElementById(picInputId).click()}>📁 Photo Upload</button>
                        {f.profilePic && (
    ...
    {errs.department && <span className="tm-form-error">{errs.department}</span>}
            ...
            <div className="tm-form-group">
                ...
                <select className="tm-form-select" value={f.type} onChange={e => upd('type', e.target.value)}>
    ...
    {errs.role && <span className="tm-form-error">{errs.role}</span>}
            ...
            <div className="tm-form-group">
                ...
                <div className="tm-time-input-group">
                    ...
                    <input className={`tm-form-input${errs.checkInTime ? ' tm-error' : ''}`} type="time" value={f.checkInTime} onChange={e => upd('checkInTime', e.target.value)} />
                </div>
                <div className="tm-time-hint">Used as scheduled check-in in Attendance</div>
            ...
            <div className="tm-form-group">
                ...
                <div className="tm-time-input-group">
                    <Icon type="clock" size={15} />
                    <input className={`tm-form-input${errs.checkOutTime ? ' tm-error' : ''}`} type="time" value={f.checkOutTime} onChange={e => upd('checkOutTime', e.target.value)} />
                </div>
                <div className="tm-time-hint">Used as scheduled check-out in Attendance</div>
    ...
    return (
        <PageWrapper pageId="interns" pageLabel="Interns" description="Manage your interns" onNavigate={onNavigate}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#2D1B38' }}>All Interns ({team.length})</h3>
                <button
                    ...
                    onMouseOver={e => { e.currentTarget.style.boxShadow = '0 4px 16px rgba(98,48,104,0.4)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                    onMouseOut={e => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(98,48,104,0.3)'; e.currentTarget.style.transform = 'translateY(0)'; }}
                >
                    <Icon type="plus" size={16} color="#F0EAF8" /> Add New Intern
                </button>
            </div>
            ...
            {filteredTeam.length === 0 ? (
                ...
                <div className="tm-cards-grid">
                    {filteredTeam.map(member => {
                        ...
                        const pc = getPerformanceColor(member.performance);
                            ...
                            <div className={`tm-emp-card${isOnLeave ? ' tm-on-leave' : ''}`} key={member.id}>
                                    ...
                                    <div className="tm-card-emp-info">
                                        ...
                                        <div className="tm-card-name-wrap">
                                            ...
                                            <div className="tm-checkin-badge">
                                                ...
                                                {isOnLeave ? 'On Leave' : `${member.checkInTime} — ${member.checkOutTime}`}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="tm-card-stats">
                                    <div className="tm-stat-item">
                                        ...
                                        <span className="tm-stat-value">{Array.isArray(member.projects) ? member.projects.length : member.projects}</span>
                                    </div>
                                    <div className="tm-stat-item">
                                        <span className="tm-stat-label"><Icon type="award" size={12} /> Performance</span>
                                        <span className="tm-stat-value" style={{ color: pc }}>{member.performance}%</span>
                                        <div className="tm-perf-bar-track"><div className="tm-perf-bar-fill" style={{ width: `${member.performance}%`, backgroundColor: pc }} /></div>
                                        <span className="tm-perf-label" style={{ color: pc }}>{getPerformanceLabel(member.performance)}</span>
                                    </div>
                                </div>
                ...
                <div className="tm-modal-overlay" onClick={() => setModalOpen(false)}>
                    <div className="tm-modal-box" onClick={e => e.stopPropagation()}>
                        <div className="tm-modal-header">
                            <div>
                                <div className="tm-modal-title">Add New Intern</div>
                                <div className="tm-modal-subtitle">Add a new intern to your team</div>
                            </div>
                        ...
                        <div className="tm-modal-body">
                            ...
                            {renderFormFields(form, updateForm, errors, 'int-pic-add-input', handlePicUpload)}
                        </div>
                        <div className="tm-info-box">
                            <div className="tm-info-box-title"><Icon type="info" size={15} color="#0D7289" /> Adding Interns</div>
                            <ul><li>All fields marked with * are required</li><li>Check-in/Check-out times appear in Attendance page</li></ul>
                        </div>
                        <div className="tm-modal-footer">
                            ...
                            <button className="tm-btn-submit" onClick={handleSubmit} disabled={submitting}>
                                {submitting ? 'Adding...' : <><Icon type="plus" size={14} color="#F0EAF8" /> Add Intern</>}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {editModal && (
                <div className="tm-modal-overlay" onClick={() => setEditModal(null)}>
                    <div className="tm-modal-box" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="tm-modal-body">
                            <div className="tm-form-section-title">Intern Information</div>
                            {renderFormFields(editForm, updateEditForm, editErrors, 'int-pic-edit-input', handleEditPicUpload)}
                        </div>
                        <div className="tm-info-box">
                            <div className="tm-info-box-title"><Icon type="info" size={15} color="#0D7289" /> Editing Intern</div>
                            <ul><li>All fields marked with * are required</li><li>Changes will be saved to Firebase immediately</li></ul>
                        </div>
                        <div className="tm-modal-footer">
